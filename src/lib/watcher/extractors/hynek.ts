...
export const HYNEK_SERIES: Record<string, { name: string; slug: string; audience?: "kids" | "mixed" | "adults" }> =
  {
    ...
    "čpXC": { name: "Český pohár MTB", slug: "cesky-pohar-mtb", audience: "mixed" },
    "čpX": { name: "ČP Cyklokros", slug: "cp-cyclocross", audience: "mixed" },
    prima: { name: "Prima Cup", slug: "primacup", audience: "mixed" },
    mtbbl: { name: "MTB Bundesliga", slug: "mtb-bundesliga", audience: "adults" },
    pmtbp: { name: "Pražský MTB pohár", slug: "prazsky-mtb-pohar", audience: "mixed" },
...
export function canonicalizeSeries(raw: string): { name: string; slug: string } | null {
  ...
  const aliases: Record<string, string> = {
    ...
    kpz: "Kolo pro život",
    "cp mtb": HYNEK_SERIES["čpXC"]!.name,
    "c p mtb": HYNEK_SERIES["čpXC"]!.name,
    "cesky pohar mtb": HYNEK_SERIES["čpXC"]!.name,
    "uci mtb world cup": "UCI MTB World Cup",
    "uci c1": "UCI / C1",
    "prazsky pohar mtb": HYNEK_SERIES.pmtbp!.name,
