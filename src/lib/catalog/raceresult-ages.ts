 ...
 * read it without one.
 *
 * Bounded like the Italian pass, and forgetful in the same way it is not: an
 * event whose contest list is empty is remembered, so the next run spends its
 * budget on events that have not been asked yet.
 */
export async function fillRaceResultAgeCategories(opts?: {
  ...
  const skip = await recentlyProbed(supabase, "raceresult_ages");
  ...
  for (const target of targets) {
    try {
      ...
      if (!ages.length) {
        result.silent += 1;
        await recordProbe(supabase, target.id, "raceresult_ages", "silent");
        continue;
      }
      ...
      if (error) {
        result.failed += 1;
        await recordProbe(supabase, target.id, "raceresult_ages", "failed");
      } else {
        result.filled += 1;
      }
    } catch {
